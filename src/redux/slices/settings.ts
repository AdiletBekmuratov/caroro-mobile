...
export const initSettings = createAsyncThunk(
  ...
  async () => {
    ...
    const firstTimeObj =
      firstTimeStr != null ? JSON.parse(firstTimeStr) : { firstTime: true };

    const vibrateString = await AsyncStorage.getItem('vibrate');
    const vibrateObj =
      vibrateString !== null ? JSON.parse(vibrateString) : { vibrate: true };
  ...
  extraReducers: builder => {
    builder
      .addCase(
        ...
        (
          state,
          action: PayloadAction<{ firstTime: boolean; vibrate: boolean }>,
        ) => {
